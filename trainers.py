...
class BasicTrainer(object):
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        all_logits = model(concatenated_batch["concatenated_input_ids"], attention_mask=concatenated_batch["concatenated_attention_mask"]).logits.to(
            torch.float32
        )
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                for eval_batch in tqdm.tqdm(self.eval_batches) if self.rank == 0 else self.eval_batches:
                    if self.local_rank is not None:
                        local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank, self.world_size, self.local_rank)
                    else:
            ...
            for microbatch_idx in range(self.config.gradient_accumulation_steps):
                if self.local_rank is not None:
                    global_microbatch = slice_and_move_batch_for_device(
                        batch, microbatch_idx, self.config.gradient_accumulation_steps, self.local_rank
                    )
                    local_microbatch = slice_and_move_batch_for_device(global_microbatch, self.rank, self.world_size, self.local_rank)
                else:
...
class FSDPTrainer(BasicTrainer):
    def __init__(
        ...
        if self.config.fsdp_shard == "full":
            shard_method = ShardingStrategy.FULL_SHARD
            print(">>>> Shard Policy: ShardingStrategy.FULL_SHARD")
        elif self.config.fsdp_shard == "hybrid":
            shard_method = ShardingStrategy.HYBRID_SHARD
            print(">>>> Shard Policy: ShardingStrategy.HYBRID_SHARD")
        elif self.config.fsdp_shard == "hybrid2":
            shard_method = ShardingStrategy._HYBRID_SHARD_ZERO2
            print(">>>> Shard Policy: ShardingStrategy._HYBRID_SHARD_ZERO2")
        else:
            print(f"Error: {self.config.fsdp_shard}")
            exit(-1)

        shared_fsdp_kwargs = dict(
            auto_wrap_policy=model_auto_wrap_policy,
            sharding_strategy=shard_method,
        ...
        print(f"With local_rank={self.local_rank} At dev({torch.cuda.current_device()}): {shared_fsdp_kwargs}")
